...
"""
VARIANT 7: Planning Memory System
HYPOTHESIS: Model performance improves when it maintains a consistent investigation plan
Based on V2 (highest scorer so far) + planning memory functionality

Strategy:
1. First turn: Ask model to create an investigation plan in <plan> tags
2. Extract the plan from first response
3. Inject plan into every subsequent prompt as a reminder
"""
...
def extract_investigation_plan(response_text: str) -> str:
    """Extract the investigation plan from the model's first response."""
    ...
    if plan_match:
        ...
        LOG.info(f"Extracted investigation plan: {plan[:200]}...")
        return plan
    
    # Try to find plan in <strategy>...</strategy> tags
    strategy_match = re.search(r'<strategy>(.*?)</strategy>', response_text, re.DOTALL | re.IGNORECASE)
    if strategy_match:
        plan = strategy_match.group(1).strip()
        LOG.info(f"Extracted investigation strategy: {plan[:200]}...")
        return plan
    
    # If no explicit tags, try to find bullet point lists that look like plans
    ...
    for line in lines:
        line = line.strip()
        # Look for plan-like bullet points
        if re.match(r'^[-*•]\s+.*(?:check|inspect|look|find|search|examine|investigate)', line, re.IGNORECASE):
            in_plan = True
            plan_lines.append(line)
        elif in_plan and re.match(r'^[-*•]\s+', line):
            plan_lines.append(line)
        elif in_plan and not line:
            continue  # Empty lines in plan
        elif in_plan:
            break  # End of bullet list
    
    if plan_lines:
        plan = '\n'.join(plan_lines)
        LOG.info(f"Extracted bullet point plan: {plan[:200]}...")
        return plan
    
    LOG.warning("No investigation plan found in first response")
    return ""


def inject_plan_reminder(inputs: str, investigation_plan: str) -> str:
    """Inject the investigation plan as a reminder at the beginning of user inputs."""
    if not investigation_plan:
        return inputs
    
    plan_reminder = f"""
**REMINDER - Your Investigation Plan:**
{investigation_plan}

**Current Task:**
"""
    
    return plan_reminder + inputs
...
class ArtsivGenerationConfig(GenerateSolutionsConfig):
    ...
    inference: InferenceConfig = field(default_factory=lambda: InferenceConfig(
        temperature=0.7,  # V2's successful setting
        top_k=0,
        top_p=0.95,
        min_p=0.0,
        random_seed=0,
        tokens_to_generate=81920,  # V2's successful setting
    ...
    # NEW: Planning memory settings
    enable_planning_memory: bool = True  # Enable the planning memory system
...
class ArtsivGenerationTask(GenerationTask):
    def __init__(self, cfg: ArtsivGenerationConfig):
        ...
        # No complex response length management - keep it simple
        super().__init__(cfg)
    ...
    async def process_single_datapoint(self, data_point, all_data):
        """Enhanced process with planning memory system."""
...
"""
            ...
            # NEW: Add planning instruction to the first turn
            if self.cfg.enable_planning_memory:
                planning_instruction = """

**SPECIAL FIRST TURN INSTRUCTION:**
Before starting your investigation, create a comprehensive plan in <plan>...</plan> tags with bullet points outlining your investigation strategy. For example:

<plan>
- Check the specific function/file mentioned in the error
- Look for similar patterns in related modules  
...
This plan will be shown to you in subsequent turns to maintain consistency in your investigation approach.
"""
                inputs += planning_instruction
                LOG.info("Added planning instruction to first turn")
        ...
        try:
            for cur_step in range(total_steps):
                ...
                # NEW: Inject plan reminder BEFORE context calculations to account for added tokens
                if (self.cfg.enable_planning_memory and 
                    self.investigation_plan and 
                    cur_step > 0 and 
                    len(data_point['turns']) > 0):
                    
                    # Find the latest turn with inputs (usually the last one)
                    for i, turn in enumerate(reversed(data_point['turns'])):
                        if isinstance(turn, dict) and turn.get('inputs', '').strip():
                            original_inputs = turn['inputs']
                            modified_inputs = inject_plan_reminder(original_inputs, self.investigation_plan)
                            turn['inputs'] = modified_inputs
                            LOG.debug(f"Injected plan reminder for step {cur_step} BEFORE context calculations")
                            break

                # Context management like V2 (AFTER plan injection to account for added tokens)
                ...
                # NEW: Extract investigation plan from first response
                if (self.cfg.enable_planning_memory and 
                    cur_step == 0 and 
                    not self.plan_extracted):
                    
                    full_response = llm_output.get('_full_generation', llm_output.get('generation', ''))
                    self.investigation_plan = extract_investigation_plan(full_response)
                    self.plan_extracted = True
                    
                    if self.investigation_plan:
                        LOG.info(f"Successfully extracted investigation plan for future turns")
                    else:
                        LOG.warning("Failed to extract investigation plan from first response")
