...
"""
VARIANT 10: Adaptive Planning with Dynamic Plan Updates
HYPOTHESIS: Allow model to refine its investigation plan as it learns more
Strategy: Extract plan from first turn + update plan whenever model creates new <plan> sections
"""
...
def extract_key_focus_areas(response_text: str) -> list:
    """Extract 2-3 key focus areas from the model's response."""
    if not response_text:
        return []
    
    # Try to find plan in <plan>...</plan> tags
    ...
    if plan_match:
        ...
        for line in lines:
            line = line.strip()
            if re.match(r'^[-*•]\s+.*(?:check|inspect|look|find|search|examine|investigate)', line, re.IGNORECASE):
                plan_lines.append(line)
        plan = '\n'.join(plan_lines[:3])  # Max 3 bullets
    
    if not plan:
        return []
    
    # Extract key focus areas - limit to 2-3 most important
    focus_areas = []
    for line in plan.split('\n'):
        ...
        if re.match(r'^[-*•]\s+', line):
            ...
            focus_areas.append(clean_line)
            
            if len(focus_areas) >= 2:  # Limit to 2 key areas max
                break
    
    LOG.info(f"Extracted {len(focus_areas)} key focus areas")
    return focus_areas


def detect_plan_update(response_text: str) -> tuple:
    """
    Detect if the model has created a new or updated plan.
    Returns: (has_plan_update, new_focus_areas)
    """
    if not response_text:
        return False, []
    
    # Look for explicit plan updates
    update_indicators = [
        r'<plan>(.*?)</plan>',
        r'(?:updated?|revised?|new)\s+(?:plan|strategy)',
        r'(?:let me|i\'ll|i will)\s+(?:update|revise|change)\s+(?:the|my)\s+(?:plan|strategy)',
        r'(?:changing|shifting|adjusting)\s+(?:the|my)\s+(?:approach|strategy|plan)'
    ]
    
    has_update = False
    
    # Check for explicit plan tags first
    plan_match = re.search(r'<plan>(.*?)</plan>', response_text, re.DOTALL | re.IGNORECASE)
    if plan_match:
        has_update = True
        LOG.info("Detected explicit plan update in <plan> tags")
    else:
        # Check for plan update language
        for pattern in update_indicators[1:]:  # Skip the <plan> pattern
            if re.search(pattern, response_text, re.IGNORECASE):
                has_update = True
                LOG.info(f"Detected plan update via pattern: {pattern}")
                break
    
    if has_update:
        # Extract new focus areas
        new_focus_areas = extract_key_focus_areas(response_text)
        return True, new_focus_areas
    
    return False, []


def inject_adaptive_focus_reminder(inputs: str, focus_areas: list, plan_version: int = 1) -> str:
    """Inject adaptive focus reminder with plan version indicator."""
    if not focus_areas:
        return inputs
    
    # Create very concise reminder with adaptive length
    focus_text = " | ".join(focus_areas)
    
    # If combined focus text is too long, truncate intelligently
    max_focus_length = 120  # Conservative limit to prevent context overflow
    if len(focus_text) > max_focus_length:
        # Try with just the first focus area
        focus_text = focus_areas[0]
        if len(focus_text) > max_focus_length:
            focus_text = focus_text[:max_focus_length-3] + "..."
    
    # Add version indicator if plan was updated
    version_indicator = f" (v{plan_version})" if plan_version > 1 else ""
    adaptive_reminder = f"\n🎯 FOCUS{version_indicator}: {focus_text}\n\n"
    
    return adaptive_reminder + inputs


@nested_dataclass(kw_only=True)
class ArtsivGenerationConfig(GenerateSolutionsConfig):
    # Based on V9 settings with adaptive planning
    ...
    # Advanced settings from V2 but simplified
    ...
    # DISABLE response length management (this was the harmful addition)
    enable_response_length_management: bool = False
    
    # NEW: Adaptive planning memory settings
    enable_adaptive_planning: bool = True  # Enable adaptive focus reminders with plan updates
    max_focus_areas: int = 2  # Limit to 2 key areas max
    enable_context_failsafe: bool = True  # Enable automatic reminder removal if context overflows
    enable_plan_evolution: bool = True  # Allow plan updates during investigation
    track_plan_history: bool = True  # Track all plan changes for analysis
...
class ArtsivGenerationTask(GenerationTask):
    def __init__(self, cfg: ArtsivGenerationConfig):
        # Simple token adjustment like V2
        super().__init__(cfg)
        self.tool_executor = ToolExecutor(cfg)
        
        # Initialize adaptive planning memory
        self.focus_areas = []
        self.plan_version = 0
        self.plan_history = []  # Track plan evolution
        self.plan_extracted = False
    ...
    async def process_single_datapoint(self, data_point, all_data):
        """Enhanced process with adaptive planning memory and failsafe."""
...
"""
            ...
            # NEW: Add adaptive planning instruction to first turn
            if self.cfg.enable_adaptive_planning:
                adaptive_planning_instruction = """

**ADAPTIVE INVESTIGATION**: Create an investigation plan in <plan>...</plan> tags with 2-3 key focus areas:
...
**IMPORTANT**: You can update this plan anytime by creating new <plan>...</plan> sections as you learn more. This helps maintain focused investigation while allowing strategy evolution.
"""
                inputs += adaptive_planning_instruction
                LOG.info("Added adaptive planning instruction to first turn")
        ...
        try:
            for cur_step in range(total_steps):
                ...
                # NEW: ALWAYS inject adaptive focus reminder BEFORE context calculations
                if (self.cfg.enable_adaptive_planning and 
                    self.focus_areas and 
                    cur_step > 0 and 
                    len(data_point['turns']) > 0):
                    
                    # Find the latest turn with inputs and add adaptive reminder
                    for turn in reversed(data_point['turns']):
                        if isinstance(turn, dict) and turn.get('inputs', '').strip():
                            original_inputs = turn['inputs']
                            modified_inputs = inject_adaptive_focus_reminder(
                                original_inputs, self.focus_areas, self.plan_version
                            )
                            turn['inputs'] = modified_inputs
                            LOG.debug(f"Injected adaptive focus reminder for step {cur_step} (plan v{self.plan_version})")
                            break

                # Context management like V9 (AFTER focus injection to account for added tokens)
                ...
                # ROBUST: Context check with automatic failsafe (same as V9)
                if ENHANCED_CONTEXT_AVAILABLE and getattr(self.cfg, 'enable_enhanced_context', True):
                    ...
                    if not will_fit:
                        # FAILSAFE: If context fails after adding focus reminder, try removing it
                        if (self.cfg.enable_context_failsafe and 
                            self.focus_areas and 
                            cur_step > 0):
                            LOG.warning(f"Context check failed with focus reminder, trying without it")
                            
                            # Remove focus reminder from the latest turn
                            for turn in reversed(data_point['turns']):
                                if isinstance(turn, dict) and turn.get('inputs', '').strip():
                                    inputs = turn['inputs']
                                    if '🎯 FOCUS' in inputs:
                                        # Remove the focus reminder line(s)
                                        lines = inputs.split('\n')
                                        filtered_lines = [line for line in lines if not line.strip().startswith('🎯 FOCUS')]
                                        turn['inputs'] = '\n'.join(filtered_lines)
                                        LOG.debug(f"Removed adaptive focus reminder to save context")
                                        break
                            
                            # Re-copy to prepared_data_point and try context check again
                            prepared_data_point = copy.deepcopy(data_point)
                            
                            # Re-run truncation if needed to rebalance after reminder removal
                            if hasattr(self.cfg, 'max_seq_length') and self.cfg.max_seq_length is not None and self.cfg.max_seq_length > 0:
                                truncation_strategy = getattr(self.cfg, 'truncation_strategy', 'sequential')
                                ...
                                if (ENHANCED_CONTEXT_AVAILABLE and 
                                    ...
                                    LOG.info(f"Re-truncation stats after reminder removal: {truncation_stats}")
                                    prepared_data_point = copy.deepcopy(data_point)
                            ...
                            if not will_fit:
                                LOG.error(f"Context check still failed after removing focus reminder: {error_msg}")
                                LOG.error(f"Context stats: {context_stats}")
                                status = "failed"
                                reason = "context_length_exceeded_proactive"
                                break
                            else:
                                LOG.info(f"Context check passed after removing focus reminder")
                        else:
                ...
                # Direct LLM call (no complex response length management)
                try:
                ...
                # NEW: Extract/update focus areas from ANY response (adaptive planning)
                if self.cfg.enable_adaptive_planning:
                    full_response = llm_output.get('_full_generation', llm_output.get('generation', ''))
                    
                    # First response: always extract plan
                    if cur_step == 0 and not self.plan_extracted:
                        self.focus_areas = extract_key_focus_areas(full_response)
                        if self.focus_areas:
                            self.plan_version = 1
                            self.plan_extracted = True
                            if self.cfg.track_plan_history:
                            ...
                            LOG.info(f"Extracted initial plan v{self.plan_version} with {len(self.focus_areas)} focus areas")
                            LOG.debug(f"Initial focus areas: {self.focus_areas}")
                        else:
                            LOG.warning("Failed to extract initial plan from first response")
                    
                    # Subsequent responses: check for plan updates
                    elif cur_step > 0 and self.cfg.enable_plan_evolution:
                        has_update, new_focus_areas = detect_plan_update(full_response)
                        if has_update and new_focus_areas:
                            old_areas = self.focus_areas.copy()
                            ...
                            if self.cfg.track_plan_history:
                                self.plan_history.append({
                                    ...
                                    'version': self.plan_version,
                                    'focus_areas': self.focus_areas.copy(),
                                    'previous_areas': old_areas,
                                    'trigger': 'plan_update'
                                })
                            LOG.info(f"Updated plan to v{self.plan_version} with {len(self.focus_areas)} new focus areas")
                            LOG.debug(f"Previous: {old_areas}")
                            LOG.debug(f"Updated: {self.focus_areas}")
        ...
        # Add adaptive planning metadata
        result = {
        ...
        # Add plan evolution metadata if enabled
        if self.cfg.track_plan_history and self.plan_history:
            result['_plan_history'] = self.plan_history
            result['_final_plan_version'] = self.plan_version
            result['_final_focus_areas'] = self.focus_areas
