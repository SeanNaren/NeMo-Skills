...
class SwarmWithReflectionConfig(GenerationTaskConfig):
    ...
    reflection_plateau_threshold: int = 3
    reflection_prompt_config: str = "eval/ioi/agent/reflection"


cs = hydra.core.config_store.ConfigStore.instance()
cs.store(name="base_swarm_with_reflection_config", node=SwarmWithReflectionConfig)


class SwarmWithReflectionTask(GenerationTask):
    def __init__(self, cfg: SwarmWithReflectionConfig):
        super().__init__(cfg)
        self.agent_prompt = get_prompt(cfg.agent_prompt_config)
        self.summary_prompt = get_prompt(cfg.summary_prompt_config)
        self.reflection_prompt = get_prompt(cfg.reflection_prompt_config)
    ...
    async def _subagent_turn(self, messages: list[dict]) -> dict:
        """Call a sub-agent model with test_solution tool only."""
        try:
            out = await self.generate_with_semaphore(
    ...
    async def _run_subagent(
        ...
        """Run a sub-agent loop: the sub-agent generates code and iterates using test_solution (sample only)."""
    ...
    def _classify_failures(self, tool_output_json: str) -> list[str]:
        """Parse eval result tool output to classify failures as CE/RE/TLE/WA."""
        failure_types = set()
        try:
            result = json.loads(tool_output_json)
        except (json.JSONDecodeError, TypeError):
            return ["UNKNOWN"]

        # Check test_case_results for explicit feedback mode
        test_case_results = result.get("test_case_results", {})
        if test_case_results:
            outputs = []
            if isinstance(test_case_results, dict):
                if "outputs" in test_case_results:
                    outputs = test_case_results.get("outputs", [])
                else:
                    for subtask_data in test_case_results.values():
                        if isinstance(subtask_data, dict):
                            outputs.extend(subtask_data.get("outputs", []))
            ...
            for o in outputs:
                ...
                if score == 1.0:
                    continue
                compile_stderr = str(o.get("compile_stderr", ""))
                run_stderr = str(o.get("run_stderr", ""))
                status = str(o.get("status", "")).lower()

                if compile_stderr or "compile" in status:
                    failure_types.add("CE")
                elif "time" in status or "tle" in status or "timeout" in run_stderr.lower():
                    failure_types.add("TLE")
                elif "runtime" in status or "signal" in run_stderr.lower() or "segfault" in run_stderr.lower():
                    failure_types.add("RE")
                else:
                    failure_types.add("WA")

        if not failure_types:
            # Fallback: check avg_score / success
            if not result.get("success", False):
                failure_types.add("WA")

        return sorted(failure_types) if failure_types else ["UNKNOWN"]

    def _build_diagnosis(self, submission_history: list[dict], best_score: float, current_step: int) -> str | None:
        """Check if reflection should fire and build the diagnosis string.

        Returns None if no reflection is needed (not at interval or not plateaued).
        """
        if current_step % self.cfg.reflection_interval != 0:
            return None

        # Need at least reflection_plateau_threshold full submissions
        if len(submission_history) < self.cfg.reflection_plateau_threshold:
            return None

        threshold = self.cfg.reflection_plateau_threshold
        recent = submission_history[-threshold:]
        older = submission_history[:-threshold]

        recent_best = max(entry["score"] for entry in recent)
        older_best = max((entry["score"] for entry in older), default=0.0)

        # If recent submissions improved over older ones, no plateau
        ...
        failure_counts = {}
        for entry in recent:
            for ft in entry.get("failure_types", []):
                failure_counts[ft] = failure_counts.get(ft, 0) + 1
        failure_lines = [f"- {ft}: {count} occurrence(s)" for ft, count in sorted(failure_counts.items())]
        failure_summary = (
            "\n".join(failure_lines) if failure_lines else "- No detailed failure classification available"
        )

        # Build approaches tried from agent names
        agents_used = []
        for entry in submission_history:
            agent = entry.get("agent_name")
            if agent and agent not in agents_used:
                agents_used.append(agent)
        approaches_tried = ", ".join(agents_used) if agents_used else "Unknown"

        recent_scores = ", ".join(f"{entry['score']:.2f}" for entry in recent)

        reflection_messages = self.reflection_prompt.fill(
            {
                "total_submissions": str(len(submission_history)),
        ...
        # fill() returns a list of messages; we want the user message content
        for msg in reflection_messages:
            if msg.get("role") == "user":
                return msg["content"]

        # Fallback: return the content of the first message
        return reflection_messages[0]["content"] if reflection_messages else None
    ...
    async def process_single_datapoint(self, data_point, all_data):
        if self.evaluator is None:
            raise ValueError(
                "SwarmWithReflection requires an evaluator supporting eval_single (set ++eval_type and ++eval_config)."
            )
        ...
        if saved_state:
            ...
            submission_history = saved_state.get("submission_history", [])
        ...
        for step in range(start_step, self.cfg.max_steps):
                ...
                if coros:
                    ...
                    for (coro_name, agent_name), tc_id, res in zip(coro_names, coro_ids, results):
                        if coro_name == "assign_task":
                            ...
                            # Return code to orchestrator for full evaluation
                            ...
                            if not res.get("sample") and res["code"]:
                                ...
                                failure_types = self._classify_failures(res["tool_out"])
            ...
            # After processing all tool calls, check if reflection should fire
            diagnosis = self._build_diagnosis(submission_history, best_score, step + 1)
            if diagnosis is not None:
                self.dp_print(data_point, f"step {step + 1}: injecting reflection")
                reflection_msg = {"role": "user", "content": diagnosis}
                agent_messages.append(reflection_msg)
                trace.append({"source": "reflection", **reflection_msg, "iteration": step + 1})
...
GENERATION_TASK_CLASS = SwarmWithReflectionTask


@hydra.main(version_base=None, config_name="base_swarm_with_reflection_config")
def swarm_with_reflection_generation(cfg: SwarmWithReflectionConfig):
    cfg = SwarmWithReflectionConfig(_init_nested=True, **cfg)
    LOG.info("Config used: %s", cfg)
    task = SwarmWithReflectionTask(cfg)
    task.generate()


HELP_MESSAGE = get_help_message(SwarmWithReflectionConfig, server_params=server_params())
...
if __name__ == "__main__":
    if "--help" in sys.argv or "-h" in sys.argv:
        ...
        setup_logging()
        swarm_with_reflection_generation()
