...
"""
VARIANT 3: Fixed Tokens Only (Conservative)
Tests if just fixing token limits + context length helps, keeping temperature=0.0
Minimal changes from current artsiv.py - only essential fixes
"""
...
class ArtsivGenerationConfig(GenerateSolutionsConfig):
    # HYPOTHESIS: Just fixing token generation + context might be enough
    inference: InferenceConfig = field(default_factory=lambda: InferenceConfig(
        temperature=0.0,  # Keep current conservative temperature
        top_k=0,
        top_p=0.95,
        min_p=0.0,
        random_seed=0,
        tokens_to_generate=81920,  # CRITICAL: Fix from 2048 to 81920
    ...
    # Keep all current settings except critical fixes
    mount_directory: str = "/repos/"
    remove_thinking: bool = True
    total_steps: int = 20

    # Repository filtering settings - keep current
    ...
    # Tool detection settings - keep current
    ...
    # CRITICAL FIX: Set context length properly
    max_seq_length: int = 262144  # Fix from None to proper value
    show_line_counts: bool = False
    max_view_lines: int = 1000

    # Keep current truncation strategy
    truncation_strategy: str = "bookend"
    
    # Keep all current advanced settings
    ...
    # Keep current response length management
