...
"""
VARIANT 19: External Memory (Built on V2)
Simple external memory system - model writes brief notes at start, we preserve them

Key features:
1. First turn asks for notes between <memory> tags
2. Simple regex extraction of content between tags
3. Appends as [Memory: ...] to all subsequent turns
4. Notes persist even when conversation is truncated
5. Ultra-simple - just extract between tags, no parsing
"""
...
def extract_memory_notes(response: str) -> str:
    """Extract memory notes from between <memory> tags."""
    import re
    
    # Simple regex to extract content between <memory> tags
    match = re.search(r'<memory>(.*?)</memory>', response, re.DOTALL)
    if match:
        memory_content = match.group(1).strip()
        # Ensure it's on one line and not too long
        memory_content = ' '.join(memory_content.split())  # Collapse whitespace
        return memory_content[:300]  # Max 300 chars to keep it concise
    return ""
...
Before investigating, write 3-5 brief notes between <memory> tags about key things to remember.
Example: <memory>Check field conversion logic | Look for enum handling | Consider to_python method</memory>

Then proceed with your investigation."""
        ...
        try:
            for cur_step in range(total_steps):
                ...
                # V19: Append external memory to turns after the first
                if external_memory and cur_step > 0 and len(data_point['turns']) > 0:
                    last_turn = data_point['turns'][-1]
                    if isinstance(last_turn, dict) and 'inputs' in last_turn:
                        memory_text = f"\n\n[Memory: {external_memory}]\n"
                        if memory_text not in last_turn['inputs']:
                            last_turn['inputs'] = last_turn['inputs'] + memory_text
                            LOG.debug(f"Appended external memory to turn {cur_step}")
                ...
                # V19: Extract memory from first response
                if cur_step == 0 and not external_memory:
                    memory = extract_memory_notes(llm_output.get('generation', ''))
                    if memory:
                        external_memory = memory
                        LOG.info(f"Extracted external memory: {memory[:100]}...")
