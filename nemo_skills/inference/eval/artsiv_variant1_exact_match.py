...
"""
VARIANT 1: Exact Match Configuration
Replicates the exact successful configuration from the logs
"""
...
class ArtsivGenerationConfig(GenerateSolutionsConfig):
    # Core inference settings - EXACT MATCH TO SUCCESSFUL RUN
    inference: InferenceConfig = field(default_factory=lambda: InferenceConfig(
        temperature=0.7,  # CRITICAL: Was 0.7 in successful run
        top_k=0,
        top_p=0.95,
        min_p=0.0,
        random_seed=0,
        tokens_to_generate=81920,  # CRITICAL: Was 81920 in successful run
    ...
    remove_thinking: bool = True  # CRITICAL: This strips thinking tags after generation
    total_steps: int = 20

    # Repository filtering settings - EXACT MATCH
    ...
    # Context settings - CRITICAL: Match successful run exactly
    max_seq_length: int = 262144  # CRITICAL: Was 262144 in successful run
    ...
    # Response length management settings - CRITICAL: Same as successful run
