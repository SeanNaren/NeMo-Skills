...
This directory contains all utility modules and documentation for the Artsiv code generation system.
...
- `artsiv_summarization.py` - Summarization code (currently disabled, preserved for future use)
...
```python
from nemo_skills.inference.eval.artsiv_utils.utils import filter_repo_dict, tree_repo_dict
from nemo_skills.inference.eval.artsiv_utils.bookend_truncation import bookend_truncate_dialogue_history
from nemo_skills.inference.eval.artsiv_utils.loop_detection import detect_repetitive_tool_calls
from nemo_skills.inference.eval.artsiv_utils.enhanced_context_management import TokenCounter
from nemo_skills.inference.eval.artsiv_utils.final_turn_prompt import inject_final_turn_instruction
from nemo_skills.inference.eval.artsiv_utils.v4.dialog_processor import DialogProcessor
from nemo_skills.inference.eval.artsiv_utils.v4.tool_executor import ToolExecutor
```

## Version Management

The main `artsiv.py` uses the `PROMPT_TEMPLATE_VERSION` variable to select which version to use:
```python
