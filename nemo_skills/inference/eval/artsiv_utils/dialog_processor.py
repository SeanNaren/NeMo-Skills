"""
Dialog processing utilities for Artsiv.

Token Counting Strategy:
- LLM Responses: Use actual token counts from llm_output['num_generated_tokens']
- Tool Outputs: Tool executor provides actual counts (uses tiktoken internally)
- Analysis/Debugging: Character-based analysis only (no token estimation)

This module does NOT estimate tokens. All token counts must come from the LLM
or tool executor. For failure analysis, we only look at character distribution.
"""
        ...
        if dict_match:
            try:
                ...
                if parsed_dict.get('type') == 'tool_calls' and 'tool_call' in parsed_dict:
                    ...
                    LOG.warning(
                        f"Detected strange format tool call (backup handling): {tool_data}. This should not happen often."
                    )
                    return {"type": "tool_calls", "tool_call": tool_data}
            except json.JSONDecodeError as e:
