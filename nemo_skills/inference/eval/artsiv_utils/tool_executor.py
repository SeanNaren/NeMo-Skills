...
class ToolExecutor:
    ...
    def execute_tool(self, extracted_block: dict, repo_dict: dict) -> Tuple[str, int]:
        """Execute a tool call and return the result with token count.

        Returns:
            Tuple of (output_content, token_count)
        """
        ...
        if not tool_name:
            if "path" in extracted_block:
                ...
                error_msg = f"Error: Cannot determine which tool to execute. Please specify 'tool' field in your JSON."
