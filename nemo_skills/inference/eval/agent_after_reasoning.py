...
class AgentAfterReasoningGenerationTask(GenerationTask):
    ...
    async def process_single_datapoint(self, data_point, all_data):
        ...
        self.dp_print(
            ...
            f"reasoning_len={len(r_msg.get('reasoning_content', ''))}",
        )
        trace.append({"source": "reasoner", **r_msg})

        # Extract initial code (check both content and reasoning_content)
        initial_code = self._extract_cpp(r_msg["content"], r_msg.get("reasoning_content"))
