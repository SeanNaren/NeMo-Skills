...
"""
VARIANT 17: Simple Plan V2 (Built on V2)
Simpler approach: Just ask for a brief plan in the first turn input
No system prompt changes, no complex tag requirements

Key features:
1. Adds "Please briefly list your investigation strategy" to first turn
2. Extracts numbered/bulleted items as the plan
3. Appends plan as a simple reminder to subsequent turns
4. Aims to solve V15's token exhaustion without V16's complexity
"""
...
def extract_simple_plan(response: str) -> str:
    """Extract a simple plan from the response - looks for numbered/bulleted lists."""
    lines = response.split('\n')
    plan_lines = []
    
    for line in lines:
        line = line.strip()
        # Look for numbered or bulleted items (including unicode bullet)
        if re.match(r'^(\d+[\.\)]|[-*•\u2022])\s+', line):
            # Keep the line as-is
            plan_lines.append(line)
        # Stop at tool calls or after collecting several items
        elif '<tool_call>' in line or len(plan_lines) > 5:
            break
    
    if plan_lines:
        return '\n'.join(plan_lines[:5])  # Max 5 items
    return ""
...
class ArtsivGenerationTask(GenerationTask):
    ...
    async def process_single_datapoint(self, data_point, all_data):
        ...
        # V17: Track the investigation plan
        investigation_plan = ""
...
Please briefly list your investigation strategy (3-5 bullet points) before starting."""
        ...
        try:
            for cur_step in range(total_steps):
                ...
                # V17: Append plan to turns after the first
                if investigation_plan and cur_step > 0 and len(data_point['turns']) > 0:
                    ...
                    if isinstance(last_turn, dict) and 'inputs' in last_turn:
                        ...
                        if plan_reminder not in last_turn['inputs']:
                            ...
                            LOG.debug(f"Appended plan reminder to turn {cur_step}")
                ...
                # V17: Extract plan from first response
                if cur_step == 0 and not investigation_plan:
                    plan = extract_simple_plan(llm_output.get('generation', ''))
                    if plan:
                        investigation_plan = plan
                        LOG.info(f"Extracted plan from first response: {plan[:100]}...")
