# Response Length Management for LocAgent

## Problem Summary

The LocAgent was failing when configured with very large token generation limits (e.g., 81,920 tokens). The model would generate exactly the maximum allowed tokens and get cut off mid-response, unable to output the required `<locations>` tags to complete the task.
