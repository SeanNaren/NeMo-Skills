...
output_dir = f"/workspace/generations/comparison/ioi25/kimi_swarm_reflection_sample_only_10steps/"
gpus = 8
...
eval(
    ctx=wrap_arguments(
        ...
        "++sample_only=True "
        "++max_time=03:45:00 "
        "++eval_config.test_file=/workspace/data/eval_datasets/ioi25/test_metadata.json "
    ),
    cluster=cluster,
    with_sandbox=True,
    expname="kimi_swarm_reflection_sample_only_ioi25",
    generation_module="nemo_skills.inference.eval.swarm_with_reflection",
    model=f"/hf_models/{model_name}",
    server_type='sglang',
    server_gpus=gpus,
    server_nodes=server_nodes,
    benchmarks="ioi25:500",
    data_dir=data_dir,
